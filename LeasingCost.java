public class LeasingCost {
    ...
    * if we want to target the data for description 2, we can first take the substring of the entire string from the letter 'E'
    * but first we need to target the index of 'E', we can do it by target the index of the string "description2:" and add 13("description2" is 13 chars long)to it
    ...
    * to extract "EFGH" (the data we want) from newString. we need to target the index of the first ';' which we can simply use newString.indexOf(';')
    ...
    public static Vehicle createVehicle(String description)
{
    ...
    String stopper = "name:";
    String name = getPairedValue(description, stopper);
   
    stopper = "type:";
    String vehicleType = getPairedValue(description, stopper);
   
    stopper = "mile/unit:";
    double usage = Double.parseDouble(getPairedValue(description,stopper));
   
    stopper = "charger:" ;
    double charger = 0.0 ;
    
    if (vehicleType.compareToIgnoreCase("electric")==0)
    {
        charger = Double.parseDouble(getPairedValue(description, stopper));
    }
    
    stopper = "due:" ;
    double dueAtSigning = Double.parseDouble(getPairedValue(description, stopper));
   
    stopper = "length:";
    int leaseLength = Integer.parseInt(getPairedValue(description, stopper));
   
    stopper = "monthly:";
    double mCost = Double.parseDouble(getPairedValue(description, stopper));
   
    stopper = "allowance:";
    int mileageAllowance = Integer.parseInt(getPairedValue(description, stopper));
   

    Fuel fuel;
    if (vehicleType.compareToIgnoreCase("gas")==0)
    {
        fuel = new Fuel(usage);
    }
    else
    ...
    Lease leaser = new Lease(dueAtSigning, leaseLength, mCost, mileageAllowance);
   
    Vehicle vehicletype = new Vehicle(name, fuel, leaser);
    ...
    */
    ...
    {
        Fuel fuelercar = v.getFuel();
        Lease leasercar = v.getLease();
    
    
        double co2PerUnit ;
        if (fuelercar.getType() == Fuel.GAS)
        {
        ...
        double C02check = computeCO2 (leasercar.getLeaseLength(), fuelercar.getUsage(), leasercar.getMileageAllowance(),co2PerUnit);
        v.setCO2Emission(C02check);
        ...
        double checkFuel = computeFuelCost(leasercar.getLeaseLength(), fuelercar.getUsage(), leasercar.getMileageAllowance(), fuelPrice);


        v.setFuelCost(checkFuel);
    
        double checkLease = computeLeaseCost(leasercar.getDueAtSigning(), leasercar.getLeaseLength(), leasercar.getMonthlyCost());


        v.setTotalCost(checkLease + checkFuel + fuelercar.getCharger());
    }
    
}
   
    public static String getPairedValue(String description, String target)
    {
    char stop = ';';
    int begin = description.indexOf(target) + target.length() ;
    int end = description.indexOf(stop, begin);


    String finalval = description.substring(begin, end);
    return finalval;
    }
